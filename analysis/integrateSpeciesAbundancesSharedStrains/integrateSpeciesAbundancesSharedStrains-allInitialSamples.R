...
dataStrainSharing <- read.table("workflow/analysis/compareSharedStrainCalls/out/strainSharingCalls-consolidated-initialX.txt",
...
dataStrainFishing <- read.table("workflow/analysis/identifySharedStrains-strainFishing/out/strainFishing-sharingAnnotated-initialX.txt.gz",
                                header=TRUE, stringsAsFactors = FALSE)
...
# List all possible sample pairs from the initial timepoint.
...
samplePairsInitialTimepoint<- samplePairsAll %>%
  filter(sample1 %in% c(samplesInitial), sample2 %in% c(samplesInitial),
         substr(sample1,1,1)=="X", substr(sample2,1,1)=="X")


# Annotate the species abundances for each within-household pair of samples.
iSamplePairs=0
speciesAbundancesAnnotatedInitialTimepoint <- 
  foreach(isample1=samplePairsInitialTimepoint$sample1,
          isample2=samplePairsInitialTimepoint$sample2, .combine="rbind") %do% {
            # Increment counter.
...
write.table(speciesAbundancesAnnotatedInitialTimepoint %>% filter(relative_abundance>0),
            gzfile(paste0(OUTDIR, "annotatedSpeciesAbundances-initialTimepoints.txt.gz")),
...
speciesAbundancesAnnotatedInitialTimepointSummary <-
  speciesAbundancesAnnotatedInitialTimepoint %>%
...
write.table(speciesAbundancesAnnotatedInitialTimepointSummary,
            paste0(OUTDIR, "annotatedSpeciesAbundances-initialTimepoints-summary.txt"),
            row.names=FALSE, quote=FALSE)
