...
dataStrainSharing <- read.table("workflow/analysis/compareSharedStrainCalls/out/strainSharingCalls-consolidated.txt",
...
dataStrainFishing <- read.table("workflow/analysis/identifySharedStrains-strainFishing/out/strainFishing-sharingAnnotated.txt.gz",
                                header=TRUE, stringsAsFactors = FALSE)
...
# Retain only sample pairs from subjects in the same household
# at which each of the samples was collected from a key timepoint
# or was the last follow-up sample.
samplePairsMajorTimepoints <- samplePairsAll %>%
  filter(sample1 %in% c(samplesKeyTimepoints, samplesInitial, samplesLastSequencedX),
         sample2 %in% c(samplesKeyTimepoints, samplesInitial, samplesLastSequencedX),
         substr(sample1,1,2)==substr(sample2,1,2))


# Annotate the species abundances for each within-household pair of samples.
iSamplePairs=0
speciesAbundancesAnnotatedMajorTimepoints <- 
  foreach(isample1=samplePairsMajorTimepoints$sample1,
          isample2=samplePairsMajorTimepoints$sample2, .combine="rbind") %do% {
            # Increment counter.
            iSamplePairs <- iSamplePairs+1
            if(iSamplePairs %% 100 == 0){
              print(paste0(iSamplePairs, " species pairs analyzed"))
            }
...
write.table(speciesAbundancesAnnotatedMajorTimepoints %>% filter(relative_abundance>0),
            gzfile(paste0(OUTDIR, "annotatedSpeciesAbundances-majorTimepoints.txt.gz")),
...
speciesAbundancesAnnotatedMajorTimepointsSummary <-
  speciesAbundancesAnnotatedMajorTimepoints %>%
...
write.table(speciesAbundancesAnnotatedMajorTimepointsSummary,
            paste0(OUTDIR, "annotatedSpeciesAbundances-majorTimepoints-summary.txt"),
            row.names=FALSE, quote=FALSE)
