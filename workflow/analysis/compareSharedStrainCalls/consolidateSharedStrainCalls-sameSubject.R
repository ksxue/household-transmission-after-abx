# This script consolidates the strain sharing calls for samples from the same subject
...
dataFixedDiffsSharing <- 
  read.table("workflow/analysis/identifySharedStrains-fixedDiffs/out/fixedDiffs-sameSubject.txt.gz",
...
dataStrainFishingSharing <-
  read.table("workflow/analysis/identifySharedStrains-strainFishing/out/strainFishing-sameSubject.txt.gz",
             header=TRUE, stringsAsFactors = FALSE)
...
write.table(dataSharedStrainsConsolidated,
            paste0(OUTDIR, "strainSharingCalls-consolidated-sameSubject.txt"),
